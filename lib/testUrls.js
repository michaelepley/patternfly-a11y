...
async function testPage({ page, data, worker }) {
  ...
  if (screenshots) {
    ...
    if (label) {
      screenshotFile += `_${label}`;
      if (crawl) {
        screenshotFile += url.replace(/[^a-zA-Z0-9]/g, "_");
      }
    } else {
      screenshotFile += url.replace(/[^a-zA-Z0-9]/g, "_");
    }
    ...
    await page.setViewport(viewportDimensions ? viewportDimensions : { width: 1920, height: 1080 });
  ...
  const axeResults = await page.evaluate(
    (context, options) => {
      console.log("Currenty page content: " + context);
...
async function testUrls(urls, options) {
  ...
  urls.forEach((url, index) => {
    cluster.queue({
      ...
      crawl: url?.crawl !== undefined ? url.crawl : options.crawl,
      waitFor: options.waitFor,
      label: typeof url === "object" ? url.label : null,
      afterNav: typeof url === "object" ? url.afterNav : null,
      screenshots: options.screenshots,
      context: url?.context ? url.context : options.context,
      viewportDimensions: url?.viewportDimensions,
    });
  });
